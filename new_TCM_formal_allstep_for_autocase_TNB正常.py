    ...
    def enter_case(self):
        ...
        for every_row in range(2,self.testplan_sheet.max_row + 1):  # 这里是匹配testplan sheet的最大行数，除去第一行表头，+1是因为range(2,9）行是循环2到8行.所以加1刚刚好
            ...
            if every_row_list[10]==None:#代表没有issue，执行pass
                ...
                time.sleep(3)#这里3秒是为了等待显示搜索后的页面，否则后面的代码会找到所有的test case
                try:#我就默认搜索case后至少有1支case出现
                    all_cases=self.driver.find_elements(By.XPATH, "//*[@id='root']/section/section/section/div/div[2]/div[3]/section/div/div/div/div/div/div/div/div[2]/table/tbody/tr")#找到搜索后出现的所有case，这里第一个tr是头标，需要去
                    len_all_cases=len(all_cases)
                    first_case = self.driver.find_element(By.XPATH, "/html/body/div[1]/section/section/section/div/div[2]/div[3]/section/div/div/div/div/div/div/div/div[2]/table/tbody/tr[2]/td[5]/div")  # 锁定正常情况下搜到case的情况，至少有一支case的情况，第4个td标签case ID，如果这里报错，代表搜到了case,但是没有not run的，plan没拉或者被打过
                    print(first_case.text)
                    if first_case.text == every_row_list[0]:  # 如果到了这步，代表上面一步pass了，说明有搜到case并至少出现一份或者直接没有搜到这份case，依然会有上一步的标签, 就是默认没搜索前的第一份case
                        ...
                        for every_case in range(2,len_all_cases+1):#，遍历所有case,除去第一个tr，因为是标头，所以从2开始，+1是因为range(2,all_cases）只能取到2到all_cases-1,不能遍历所有搜索到的case
                            ...
                            edit_button.click()
                            Execution_Status = WebDriverWait(self.driver, 10).until(ec.presence_of_element_located((By.XPATH,"/html/body/div/section/section/section[3]/div/div[1]/div[2]/div/div/div/h4[1]/div/span")))
                            Execution_Status.click()
                            set_all = WebDriverWait(self.driver, 10).until(ec.presence_of_element_located((By.XPATH,"/html/body/div/section/section/section[3]/div/div[1]/div[4]/button[1]/span")))
                            set_all.click()
                            Status = WebDriverWait(self.driver, 100).until(ec.presence_of_element_located((By.XPATH, "/html/body/div[2]/div/div[2]/div/div[2]/div[2]/div/div/div/span[1]/input")))
                            self.driver.execute_script("arguments[0].removeAttribute('readonly');", Status)
                            Status.send_keys('Pass')
                            time.sleep(1)
                            Status.send_keys(Keys.ARROW_DOWN)  # 输入pass后出现not start和pass,按一下下键选pass
                            time.sleep(1)
                            Status.send_keys(Keys.ENTER)
                            self.driver.find_element(By.XPATH,"/html/body/div[2]/div/div[2]/div/div[2]/div[3]/button[2]").click()  # 点apply
                            time.sleep(1)
                            config=self.driver.find_element(By.XPATH, "/html/body/div/section/section/section[2]/div/div/div/div/div/div/div/div/div/table/tbody/tr/td[6]/div")#获取config 标签
                            ...
                            start_time=self.driver.find_element(By.XPATH,"/html/body/div/section/section/section[3]/div/div[1]/div[4]/section[1]/div/div[3]/div/div/div[2]/div/input")#找到开始时间
                            ...
                            end_date=self.driver.find_element(By.XPATH,"/html/body/div/section/section/section[3]/div/div[1]/div[4]/section[1]/div/div[4]/div/div/div[1]/div/input")#找结束日期
                            end_date.click()
                            ...
                            end_time=self.driver.find_element(By.XPATH,"/html/body/div/section/section/section[3]/div/div[1]/div[4]/section[1]/div/div[4]/div/div/div[2]/div/input")#找结束时间
                ...
                try:#我就默认搜索case后至少有1支case出现
                    ...
                    if first_case.text == every_row_list[0]:  #如果到了这步，代表上面一步pass了，说明有搜到case并至少出现一份或者直接没有搜到这份case，依然会有上一步的标签, 就是默认没搜索前的第一份case
                        # 如果相等代表是搜到的情况
                        for every_case in range(2,len_all_cases+1):#，遍历所有case,除去第一个tr，因为是标头，所以从2开始，+1是因为range(2,all_cases）只能取到2到all_cases-1,不能遍历所有搜索到的case
                            case = self.driver.find_element(By.XPATH,f"/html/body/div[1]/section/section/section/div/div[2]/div[3]/section/div/div/div/div/div/div/div/div[2]/table/tbody/tr[{every_case}]/td[18]")  # 点击执行case
                            self.driver.execute_script("arguments[0].scrollIntoView();", case)  # 移到元素可见
                            case.click()
                            popup_windows = self.driver.window_handles[-1]  # 跳转到新页面执行准备case
                            self.driver.switch_to.window(popup_windows)  #跳转到新页面执行准备case
                            edit_button=WebDriverWait(self.driver, 10).until(ec.presence_of_element_located((By.XPATH,"/html/body/div/section/section/div[1]/div[2]/button[1]/span")))
                            ...
                            self.driver.find_element(By.XPATH,"/html/body/div[2]/div/div[2]/div/div[2]/div[3]/button[2]").click()  # 点apply
                            time.sleep(1)
                            first_step = WebDriverWait(self.driver, 10).until(ec.presence_of_element_located((By.XPATH,"/html/body/div/section/section/section[3]/div/div[1]/section/div/div[1]/div/div[1]/div/label[3]/span[1]/input")))  # 因为是fail所以第一步打issue
                            self.driver.execute_script("arguments[0].click();", first_step)
